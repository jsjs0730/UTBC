...
<form role="form" id="modifyForm" action="modifyPage" method="post">
	...
	<div class="box-body">
		...
		<div class="form-group">
			...
			<textarea rows="30" class="form-control" id="content" name="content">${boardVO.content }</textarea>	
		</div>
		<div class="form-group">
			<label for="usernick">Writer</label>
			<input type="text" name="usernick" class="form-control" value="${boardVO.usernick }" readonly="readonly">	
		</div>		
	</div><!-- .box-body -->
	<div class="form-group">
		<label>파일 업로드</label>
...
$(".uploadedList").on("click", ".delbtn", function(event){
	...
	if(confirm("삭제하시겠습니까???") == true){
	    ...
	    $.ajax({
			url:"/deleteFile",
			type:"post",
			data: {fileName:$(this).attr("href"),csrf:{"paramterName":csrfParameter, "headerName": csrfHeader, "token" : csrfToken}},
			dataType:"text",
			beforeSend:function(xhr){
			    xhr.setRequestHeader(csrfHeader, csrfToken);
